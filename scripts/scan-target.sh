...
Использование: ./reality-target-scan.sh [опции]
  ...
  --reuse               Не сканировать заново, использовать results.csv
...
echo "  ${C_R}ФАНТОМ${C_0}        — сертификат здесь, а сам сайт живёт в другом месте."
...
BEST="$(sort -k1,1nr -k2,2n "$RESULT_FILE" 2>/dev/null | awk -F'\t' '$6=="EXACT" || $6=="SUBNET"' | head -n 1)"
if [[ -n "$BEST" ]]; then
    B_DOM="$(echo "$BEST" | cut -f9)"
    B_IP="$(echo "$BEST" | cut -f10)"
    echo ""
    echo "📋 Готовый фрагмент для realitySettings:"
    echo ""
    cat <<JSONEOF
  "realitySettings": {
    "show": false,
    "target": "${B_DOM}:443",
    "serverNames": ["${B_DOM}"],
    "privateKey": "<СГЕНЕРИРОВАТЬ: xray x25519>",
    "shortIds": ["<СГЕНЕРИРОВАТЬ>"],
    "spiderX": "/"
  }
JSONEOF
    echo ""
    echo "  Сосед по подсети: $B_IP"
    echo "  ${C_Y}Проверьте вручную перед боем:${C_0} откройте https://${B_DOM} в браузере."
    echo "  Если это заглушка хостера, пустая страница или ошибка — цель слабая,"
    echo "  берите следующую из таблицы."
fi

if [[ "$KEEP_CSV" -eq 1 ]]; then
    echo ""
    echo "💾 Сырые результаты сохранены: $CSV"
    echo "   Переанализ без пересканирования: $0 --reuse"
fi
